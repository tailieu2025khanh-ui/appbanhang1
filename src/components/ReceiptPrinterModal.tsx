...
export const ReceiptPrinterModal: React.FC<ReceiptPrinterModalProps> = ({
  ...
  const copies = Math.max(1, storeConfig.printCopies || 1);
  ...
  const renderSingleReceipt = (copyIndex: number) => {
    const copyLabel =
      copies > 1
        ? copyIndex === 0
          ? '--- BẢN CHÍNH (GIAO KHÁCH HÀNG) ---'
          : copyIndex === 1
          ? '--- BẢN SAO 1 (CỬA HÀNG GIỮ) ---'
          : copyIndex === 2
          ? '--- BẢN SAO 2 (BẾP / PHA CHẾ) ---'
          : `--- BẢN IN LẦN ${copyIndex + 1}/${copies} ---`
        : null;

    return (
      <div
        key={copyIndex}
        className={`bg-[#FAF9F6] text-[#1A1A1A] p-5 shadow-sm font-mono text-xs rounded-sm border border-[#E0E0D6] relative ${
          is58mm ? 'w-[280px]' : 'w-[340px]'
        } ${copyIndex > 0 ? 'mt-6 pt-6 border-t-2 border-dashed border-gray-400' : ''}`}
        style={{ fontFamily: "'Courier New', Courier, monospace" }}
      >
        {copyLabel && (
          <div className="text-center font-bold text-[10px] bg-[#E0E0D6] p-1 rounded-sm mb-3 uppercase tracking-wider text-[#5A5A40]">
            {copyLabel}
          </div>
        )}
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-[9px] text-[#808070]">Phần mềm BÁN HÀNG CHẢ GIÒ POS</p>
        </div>
      </div>
    );
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#2C2C24]/60 backdrop-blur-xs p-4 animate-fadeIn">
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full max-h-[92vh] flex flex-col overflow-hidden border border-[#E0E0D6]">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[11px] text-amber-300 font-medium">
                Số bản in thiết lập: <span className="font-extrabold">{copies} bản</span>
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 bg-[#F5F5F0] overflow-y-auto flex-1 flex flex-col items-center">
          <div id="printable-receipt" className="flex flex-col items-center">
            {Array.from({ length: copies }).map((_, index) => renderSingleReceipt(index))}
          </div>
        </div>
        ...
        <div className="p-4 bg-[#FAF9F6] border-t border-[#E0E0D6] flex items-center justify-between gap-3">
          ...
          >
            ...
            <span>IN HÓA ĐƠN ({copies} BẢN) NGAY</span>
          </button>
        </div>
      </div>
    </div>
